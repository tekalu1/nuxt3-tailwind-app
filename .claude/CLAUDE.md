...
| エージェント | 用途 |
|-------------|------|
| `planner` | ヒアリング（初期・確認）+ 事前調査（現状分析・影響範囲特定・技術調査）+ 実装計画作成、docs/plans/配下へのドキュメント作成 |
...
**重要:** 改修案件や新機能追加の場合、**まず `planner` でヒアリング・調査・計画作成を行ってください。**
plannerは以下の4フェーズを実行します：
1. **Phase 0: 初期ヒアリング**: ビジネス目的・スコープ・受け入れ基準を確認（選択肢ベース）
2. **Phase A: 事前調査**: 現状分析、影響範囲特定、技術調査、リスク特定
3. **Phase A': 確認ヒアリング**: 調査結果を踏まえた技術選択・スコープ調整の確認
4. **Phase B: 計画作成**: 調査・ヒアリング結果に基づくタスク分割

これにより以下の完全な開発フローが確立されます：

```
planner（ヒアリング + 調査 + 計画作成）
    ├── Phase 0: 初期ヒアリング → レビュー
    ├── Phase A: 事前調査
    ├── Phase A': 確認ヒアリング → レビュー
    └── Phase B: 計画作成 → レビュー
    ↓
...
良い例（ヒアリング→計画→要件定義→テスト→実装→受け入れの完全フロー）:
ユーザー: "ユーザー認証機能を追加したい"
→ planner で初期ヒアリング→調査→確認ヒアリング→計画作成
→ requirements-engineer で要件定義 → test-engineer でテストシナリオ
